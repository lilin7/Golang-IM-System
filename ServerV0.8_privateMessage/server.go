...
func (this *Server) Handler(conn net.Conn) {
	...
	isActive := make(chan bool) //a channel to trace if a user is active or idle
	...
	//user timeout force quit
	for {
		select {
			case <- isActive: //current user is active, reset timer
				//do nothing, wait to reset timer
			case <- time.After(time.Second * 300): //timeout
				user.SendMessage("You are idle for too long, force quit.")
				close(user.C) //close channel C
				conn.Close()
...
func (this *Server) Start() {
	...
	go this.ListenMessage() //inside is a for loop to listen to "Message" (from broadcast or sendmsg) forever
